...
bool Level::RespondToInput(char input)
{
    ...
    std::pair<int, int> prey_position = m_board->GetPrey()->GetPosition();
    switch (input)
    {
        case 'q': //quit       
            m_playing = false;
        ...
        case 's': //go down
            ...
            legal_move = m_board->GetMonster()->MoveDown(prey_position);
            break;
        
        case 'w': //go up
            //update snake position
            legal_move = m_board->GetMonster()->MoveUp(prey_position);
            break;


        case 'a': //go left
            //update snake position
            legal_move = m_board->GetMonster()->MoveLeft(prey_position);
            break;

        case 'd': //go right
            //update snake position
            legal_move = m_board->GetMonster()->MoveRight(prey_position);
            break;
...
bool Level::StartLevel()
{
    ...
    do
    {
        ...
        {  
            ...
            {
                if(m_board->GetMonster()->GetHead() == m_board->GetPrey()->GetPosition())
                {
                    m_board->PlacePrey(m_board->GetPrey());
                }
