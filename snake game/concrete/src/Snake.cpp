...
bool Snake::MoveUp()
{
    ...
    if(m_head.first == 1 || SearchValue(ROW, m_head.first -1))
    {
        return false;
    }
...
{
    //check head not touching border nor self
    if(m_head.first == m_rows -2 || SearchValue(ROW, m_head.first + 1))
    {
        return false;
    }
    else    //move snake and update head and tail
...
{
    //check head not touching border nor self
    if(m_head.second == 1 || SearchValue(COL, m_head.second - 1))
    {
        return false;
    }
    else    //move snake and update head and tail
...
{
    //check head not touching border nor self
    if(m_head.second == m_cols -3 || SearchValue(COL, m_head.second + 1))
    {
        return false;
    }
    else    //move snake and update head and tail
